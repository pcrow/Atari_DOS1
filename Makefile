...
dos1dos-clean.asm: dos1dos.asm
	sed -e 's/\t...*\t.*\t/&;/' -e 's/\t=*\t[^.].*\t/&;/' -e 's/\t[*]=.*\t/&;/' -e 's/HILO/;&/' -e 's/[.]IF/;&/' -e 's/[.]END/;&/' -e 's/[.]TITLE/;&/' -e 's/^&/;&/' -e "s/#'\(.\)/#"'"\1"/' -e "s/'0/"'"0"/'  -e 's/ASL\tA/ASL/' -e 's/LSR\tA/LSR/' -e 's/ROL\tA/ROL/' -e 's/ROR\tA/ROR/' -e 's@.dbyte\t\([^\t;]*\)@.byte (\1)/256,-256*((\1)/256)+(\1)\t;&@' dos1dos.asm > dos1dos-clean.asm

dos1dos.bin: dos1dos-clean.asm
	xa -E -XMASM -P dos1dos.lst -o dos1dos.bin dos1dos-clean.asm

test: dos1dos.bin dos1.bin
	cmp --verbose dos1_nodup.bin dos1dos.bin
	cmp --verbose dos1orig.bin dos1.bin
